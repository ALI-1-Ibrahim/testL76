...
maxroom = Math.max(...rooms);
minroom = Math.min(...rooms);
...
maxbathroom = Math.max(...bathrooms);
minbathroom = Math.min(...bathrooms);
...
.then(function(d) {
  ...
  for (i = 0; i < json.length; i++) {
    if (json[i].price <= slider_price.value) {
      if (json[i].number_of_rooms == slider_room.value) {
        if (json[i].number_of_bathrooms == slider_bathroom.value) {
          if(json[i].gas==cgas){
